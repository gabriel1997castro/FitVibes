-- Corrigir ambiguidade de payment_cycle na trigger de balances

DROP FUNCTION IF EXISTS public.upsert_balance_on_payment_history CASCADE;
...
DECLARE
  cycle_start DATE;
  cycle_end DATE;
  v_payment_cycle TEXT;
  existing_balance_id UUID;
BEGIN
  -- Buscar tipo de ciclo do grupo
  SELECT payment_cycle INTO v_payment_cycle FROM groups WHERE id = NEW.group_id;

  IF v_payment_cycle = 'weekly' THEN
  ...
  ELSIF v_payment_cycle = 'monthly' THEN
